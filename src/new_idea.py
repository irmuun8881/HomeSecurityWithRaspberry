...
def get_face_encoding(face_image):
    ...
    dlib_image = dlib.array_to_img(face_image)
    detector = dlib.get_frontal_face_detector()
    detected_faces = detector(dlib_image, 1)
    if detected_faces:
        shape = dlib.shape_predictor()(dlib_image, detected_faces[0])
        face_descriptor = face_recognizer.compute_face_descriptor(dlib_image, shape)
...
def compare_faces(known_encodings, face_encoding_to_check):
    if face_encoding_to_check is not None:
        ...
        if distances[best_match_index] < 0.6:  # Threshold for recognizing a face
...
if __name__ == '__main__':
    ...
    while time.time() - start_time < 20:  # Run for 20 seconds
        ...
        if tracking:
            ...
            if len(faces) > 0:
                frame_queue.put((frame, gray, faces[0]))  # Process only the first detected face for simplicity
                tracking = True
